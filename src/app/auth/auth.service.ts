...
export class AuthService {
  baseUrl = 'https://identitytoolkit.googleapis.com/v1/accounts:action?key=AIzaSyB_lHzVMi6GQSiMp8zr48Te7ZjGfapvECg';
  ...
  login(email: string, password: string): Observable<AuthResponse> {
    return this.post('signInWithPassword', email, password)
  ...
  private handleAuthentication(response: AuthResponse) {
    ...
    const user = new User(response.email, response.localId, response.idToken, expiresAt);
  ...
  autoLogin() {
    ...
    const loadedUser = new User(user.email, user.id, user._token, tokenExpiresAt);
    if (loadedUser.token) {
      const expirationDuration = tokenExpiresAt.getTime() - new Date().getTime();
  ...
  logout() {
    ...
    this.tokenExpirationTimeoutId && clearTimeout(this.tokenExpirationTimeoutId);
    this.tokenExpirationTimeoutId = undefined;
  }

  autoLogout(expirationDuration: number) {
    this.tokenExpirationTimeoutId = setTimeout(this.logout.bind(this), expirationDuration);
  }
}
